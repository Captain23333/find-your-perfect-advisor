...
这是本机 Node.js 版本过旧触发的已知兼容性问题，和网络、权限、CV、项目配置都无关。旧版本 Node 在 macOS 上启动子进程时使用的一个系统底层开关不被支持，系统会直接拒绝并返回 “EINVAL”；新版 Node 已经修复了这个问题，升级后不会再出现。

**解决方法：升级 Node.js 到最新 LTS 版本，然后重启本地控制台。** 任选以下一种方式即可：

**方式一：手动下载安装（推荐给不熟悉命令行的用户）**

1. 打开浏览器访问 [nodejs.org](https://nodejs.org)，网站会自动识别你的系统，点击首页的绿色按钮下载 **LTS** 版安装包（Mac 是 `.pkg`，Windows 是 `.msi`）
2. 双击下载好的安装包，像安装普通软件一样一路“下一步”即可，无需使用 Homebrew、nvm 或任何命令行工具

**方式二：终端命令更新（适合已经装过 Homebrew 或 nvm 的用户）**

- 用 Homebrew：`brew upgrade node`
- 用 nvm：`nvm install --lts`

**升级完成后（两种方式通用）：**

3. **关闭**原来运行本地控制台的终端窗口，重新打开一个**全新**的终端窗口
4. 重新执行本地启动命令，再次尝试运行任务
