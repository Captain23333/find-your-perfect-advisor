...
6. 选择均衡、稳妥优先或冲刺优先的申请组合，并设置 Phase 1 希望保留的导师数量（默认 Top 10）。
7. 保存申请资料。
8. 随时选择本机已登录的 Codex 或 Claude Code。
9. 点击“开始寻找导师”。
10. 最迟在 shortlist 的客观申请条件筛选前补齐目标学位和申请季。
11. 选择重点导师，并勾选需要的背景调查维度。
12. P2 显示“已完成”后直接查看背调结果；需要补充维度时再重新运行。
13. 生成综合排名。网页只展示前三名，完整信息在当前项目 `outputs/` 下自动生成的 Excel 中。
...
如果 Agent 判断 CV 缺失、不可读取或明确不是真实申请者材料，任务会进入“等待补充资料”。直接在运行面板上传替换 CV 并点击“保存并继续本轮”即可；Web 会继续原来的 run 和 Agent 会话，不会从头重跑已经完成的检查或检索。
...
- 用确定性脚本控制 shortlist 与冲刺比例，并保留排除项和 matching audit
- 记录来源并生成匹配结果

Finder 浏览导师或项目页面时已经发现的信息会立即保存；后续只查询缺失、过期或冲突字段。同一项目的信息只查一次，再关联到多位导师。

候选表中的“综合匹配”由确定性脚本按 `60% 研究匹配 + 40% CV 履历匹配`
...
`advisor-evaluator` 分开汇总研究匹配、履历匹配、硬条件、申请路径、机会证据、客观申请可行性和所选背调维度，生成申请就绪总表。硬条件失败、不符合客观资格或已确认关闭的机会不会因综合分高而进入主申。
...
另外请确认 Codex 或 Claude 至少有一个显示为“可用”。如果 CV 上传后被移动、删除，或 Agent 发现它明确是测试/占位材料，Finder、RP 和套磁信都会暂停并显示替换 CV 控件。上传后从同一会话继续，不需要重新启动整轮任务。RP 或套磁信缺少申请者姓名时也会明确要求补齐，不会生成示例署名成品。
